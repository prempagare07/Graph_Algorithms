...
/* defination and declaration of all requied data types and their wrapper functions*/
...
Graph* getGraphPtr(char name[]);
void printGraph(char name[]);
void createUndirectedGraph(Edge edges[], int n);
void createDirectedGraph(Edge edges[], int n);
void addNode(char name[]);
int deleteLL(Node **head_ref, int key);
void deleteNode(char name[]);
void addEdge(char name[]);
void deleteEdge(char name[]);
void traverse(Graph* graph, int i);
void display_dft(char name[]);
void display_bft(char name[]);
void topological_sort(char name[]);
void traverse_is_connected(int arbitary_node ,int A[][N], int visited[]);
void is_connected(char name[]);
int travere_search_dfs(Graph *graph, int node ,int i);
int search_dfs(char name[], int node);
int search_bfs(char name[], int node);
int traverse_is_cyclic(Graph *graph, int node ,int came_from , int visited[]);
int is_cyclic(char name[]);
void display_mst(char name[]);
...
void all_possible_paths(int source , int dest); 

/*definations of all functions*/

Graph *all_graphs_ptr;
...
void is_connected(char name[])
{
	//Used adjacency matrix to reduce complexity
	Graph *graph = getGraphPtr(name);
	int visited[N] , visited_reversed[N];
	int A[N][N] , A_T[N][N]; //A transpose
	Node* current;
